...
class TestViews:
    ...
    def test_contatos_post_valid_with_send_mail(self, client, monkeypatch):
        """Testa envio de formulário de contato onde send_mail é chamado dentro do bloco try"""
        # Monitorar a chamada para send_mail
        mock_send = MagicMock()
        monkeypatch.setattr('app.views.send_mail', mock_send)
        
        # Desativando o bloco de código que está sendo comentado no original
        # Isso vai fazer com que o código na view realmente chame o mock send_mail
        monkeypatch.setattr('app.views.settings.DEFAULT_FROM_EMAIL', 'test@example.com')
        monkeypatch.setattr('app.views.settings.CONTACT_EMAIL', 'contact@example.com', raising=False)
        
        data = {
            'nome': 'Visitante Completo',
            'email': 'visitante@teste.com',
            'mensagem': 'Esta é uma mensagem de teste completa.'
        }
        response = client.post(reverse('contato'), data)
        
        # Verificar a resposta e as mensagens de sucesso
        assert response.status_code == 200
        messages = list(get_messages(response.wsgi_request))
        assert any('sucesso' in str(message).lower() for message in messages)
    
    def test_form_post_valid_with_send_mail(self, client, monkeypatch):
        """Testa envio de formulário de emprego onde send_mail é chamado dentro do bloco try"""
        # Monitorar a chamada para send_mail
        mock_send = MagicMock()
        monkeypatch.setattr('app.views.send_mail', mock_send)
        
        monkeypatch.setattr('app.views.settings.DEFAULT_FROM_EMAIL', 'test@example.com')
        monkeypatch.setattr('app.views.settings.HR_EMAIL', 'hr@example.com', raising=False)
        
        data = {
            'nome_c': 'Candidato Completo',
            'email': 'candidato@teste.com',
            'experiencia': '5 anos de experiência em mecânica automotiva e elétrica'
        }
    ...
    def test_form_with_exception_logging(self, client, monkeypatch):
        """Testa se exceções são logadas adequadamente na view form"""
        # Criando um mock para o logger
        mock_logger = MagicMock()
        monkeypatch.setattr('app.views.logger', mock_logger)
        
        # Simular uma exceção no envio de e-mail
        def mock_raise_exception(*args, **kwargs):
            raise Exception("Erro simulado no envio de e-mail")
        
        monkeypatch.setattr('app.views.send_mail', mock_raise_exception)
        
        data = {
            'nome_c': 'Candidato Exceção',
            'email': 'candidato@teste.com',
            'experiencia': 'Teste de exceção'
        }
        
        # A view captura a exceção, então não devemos esperar uma exceção aqui
        response = client.post(reverse('formulario'), data)
        
        # A exceção deve ser logada no logger
        assert mock_logger.error.called
        # Verificar que a resposta contém mensagem de erro
        messages = list(get_messages(response.wsgi_request))
        assert any('erro' in str(message).lower() for message in messages)
        
    def test_contatos_with_exception_logging(self, client, monkeypatch):
        """Testa se exceções são logadas adequadamente na view contatos"""
        # Criando um mock para o logger
        mock_logger = MagicMock()
        monkeypatch.setattr('app.views.logger', mock_logger)
        ...
        data = {
            'nome': 'Contato Exceção',
            'email': 'contato@teste.com',
            'mensagem': 'Teste de exceção'
        }
